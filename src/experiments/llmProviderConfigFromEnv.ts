 ...
 * - `EEP_LLM_PROVIDER`: `"anthropic"` (Claude) or `"openai-compatible"` (ChatGPT, Gemini's
 *   OpenAI-compatibility endpoint, or a local OpenAI-compatible server such as Ollama/LM Studio).
 * - `EEP_LLM_MODEL`: the model name/id, always required.
...
export function llmProviderConfigFromEnv(env: NodeJS.ProcessEnv = process.env): LlmProviderConfig {
  ...
  throw new MissingLlmConfigError(
    `EEP_LLM_PROVIDER must be "anthropic" or "openai-compatible" (got ${provider === undefined ? 'unset' : JSON.stringify(provider)}).`,
  );
}
